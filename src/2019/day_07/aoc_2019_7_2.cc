...
class IntCodeInterpreter {
   public:
    ...
    void SetInputSignal(int input_signal) { input_signal_ = input_signal; }

    bool IsHalted() const { return is_halted_; }
...
int FindOptimalAmpConfig(const std::vector<int>& tokens) {
    ...
    int input_signal = 0;
    do {
        std::vector<IntCodeInterpreter> amps;
        for (const int& phase : phases) {
            amps.emplace_back(tokens, phase, 0);
        }

        std::size_t i = 0;
        while (!std::all_of(
            amps.begin(), amps.end(),
            [](const IntCodeInterpreter& amp) { return amp.IsHalted(); })) {
            amps[i].SetInputSignal(input_signal);
            amps[i].Run();
            input_signal = amps[i].GetOutput();

            i = (i + 1) % amps.size();
        }
        max_output = std::max(max_output, amps.back().GetOutput());

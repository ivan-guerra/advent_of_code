...
struct Bot {
  ...
  void InsertChip(int val) {
    if (std::find(std::begin(chips), std::end(chips), val) != std::end(chips)) {
      return;
    }
    chips.push_back(val);
  }
...
int GetTargetBot(const std::unordered_map<int, Bot>& robots, int target_low,
  ...
  for (const auto& [bot_id, robot] : robots) {
    if ((robot.GetLow() == target_low) && (robot.GetHigh() == target_high)) {
      return bot_id;
    }
  }
  return -1;
    ...
    for (const InstructionPtr& inst : instructions) {
      switch (inst->type) {
        case InstructionType::kInput:
          input_inst = std::dynamic_pointer_cast<InputInstruction>(inst);
          robots[input_inst->dst_bot].InsertChip(input_inst->value);
          break;
        case InstructionType::kOutput:
          output_inst = std::dynamic_pointer_cast<OutputInstruction>(inst);

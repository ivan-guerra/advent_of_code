...
std::string GetStretchedHash(const std::string& secret, int stretch = 2016) {
    std::string hash = GetMD5Hash(secret);
    while (stretch--) {
...
int GetOtpKeyIndex(const std::string& salt) {
    ...
    for (int i = 0; i < 1001; ++i) {
        std::string hash = GetStretchedHash(salt + std::to_string(index));
    ...
    while (num_keys < kTargetNumKeys) {
        ...
        std::string new_hash = GetStretchedHash(salt + std::to_string(index));

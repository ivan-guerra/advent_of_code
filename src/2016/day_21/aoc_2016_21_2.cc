...
std::string ReverseScrambling(const std::vector<Instruction>& instructions,
                              const std::string& password) {
  Scrambler pass_scrambler;
  std::string candidate = password;
  std::sort(candidate.begin(), candidate.end());
  do {
    if (pass_scrambler.Scramble(instructions, candidate) == password) {
      return candidate;
    }
  } while (std::next_permutation(candidate.begin(), candidate.end()));
  return ""; /* only reachable if given pass is not a valid scrambled pass */
}
...
int main(int argc, char** argv) {
  ...
  const std::string kPassword("fbgdceah");
  std::cout << "Answer: " << ReverseScrambling(instructions, kPassword)
            << std::endl;

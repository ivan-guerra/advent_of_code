   ...
   public:
    ...
    bool IsComplete() const {
        for (int i = 0; i < floors_.size() - 1; i++) {
            bool floor_is_clear =
                std::all_of(floors_[i].begin(), floors_[i].end(),
   ...
   private:
    bool StepItem(ElevatorDir dir, int item_index) {
        if (ElevatorDir::kUp == dir) {
            ...
            std::swap(floors_[level_][item_index],
                      floors_[level_ + 1][item_index]);
        } else {
            if ((level_ - 1) < 0) {
                return false;
            }
...
int GetMinStepsToCompleteAssembly(const AssemblySim::Floors& floors) {
    ...
    while (true) {
        ...
        for (int i = 0; i < num_nodes; ++i) {
            ...
            auto double_item_combos =
                NChooseK(curr.GetCurrFloorItemIndices(), 2);
            ...
            for (const std::vector<int>& combo : combos) {
                ...
                if (combo.size() == 2) {
                    if (candidate_up.Step(AssemblySim::ElevatorDir::kUp,
                                          combo[0], combo[1]) &&
                        !candidate_up.IsInvalid()) {
                        sims.push(candidate_up);
                    }
                    if (!candidate_down.FloorsBelowAreEmpty() &&
                        candidate_down.Step(AssemblySim::ElevatorDir::kDown,
                                            combo[0], combo[1]) &&
...
int main() {
    ...
    const int kNumItemsPerFloor = 10;
    ...
    std::cout << "Answer: " << GetMinStepsToCompleteAssembly(floors)
              << std::endl;

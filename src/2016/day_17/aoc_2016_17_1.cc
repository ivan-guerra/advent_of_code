...
std::vector<MazeLocation> GetNeighbors(const MazeLocation& loc,
  ...
  if (kOpenDoor.count(hash[0]) && ((loc.y - 1) >= 0)) {
    neighbors.emplace_back(loc.path + std::string(1, 'U'), loc.x, loc.y - 1);
  }
  if (kOpenDoor.count(hash[1]) && ((loc.y + 1) < maze_y_bound)) {
    neighbors.emplace_back(loc.path + std::string(1, 'D'), loc.x, loc.y + 1);
  }
  if (kOpenDoor.count(hash[2]) && ((loc.x - 1) >= 0)) {
    neighbors.emplace_back(loc.path + std::string(1, 'L'), loc.x - 1, loc.y);
  }
  if (kOpenDoor.count(hash[3]) && ((loc.x + 1) < maze_x_bound)) {
    neighbors.emplace_back(loc.path + std::string(1, 'R'), loc.x + 1, loc.y);
  }

  return neighbors;
}

std::string GetShortestPathToVault(const std::string& passcode,
  ...
  return ""; /* unreachable given a valid passcode */
}
...
int main(int argc, char** argv) {
  ...
  std::cout << "Answer: " << GetShortestPathToVault(passcode) << std::endl;

...
int GetNumCharsToTxStart(const std::string& signal, int window_size) {
...
int main(int argc, char** argv) {
  ...
  const int kStartSeqSize = 4;
  std::cout << "There are " << GetNumCharsToTxStart(signal, kStartSeqSize)
            << " chars until beginning of transmission." << std::endl;

...
FileSysNodePtr ConstructFileSysTree(const std::string& input) {
  ...
  while (!session_data.empty()) {
    if (IsCommand(session_data.front())) {
      std::vector<std::string> cmd_tokens = SplitOnSpace(session_data.front());
      if (cmd_tokens[1] == kChangeDir) {
        ...
        std::cerr << "unknown command: " << session_data.front() << std::endl;
      }
    }
  }
...
int GetSizeSum(FileSysNodePtr root, int limit) {
  std::queue<FileSysNodePtr> nodes;
  nodes.push(root);

  // Traverse the tree in BFS fashion maintaining the sum of directories
  // where dir->size <= limit.
  int sum = 0;
  while (!nodes.empty()) {
    ...
    for (int i = 0; i < level_size; ++i) {
      ...
      if (curr->size <= limit) sum += curr->size;
...
int main(int argc, char** argv) {
  ...
  const int kDirSizeLimit = 100000;
  std::cout << "Computed Dir Size Sum: " << GetSizeSum(root, kDirSizeLimit)
            << std::endl;

...
int main(int argc, char** argv) {
  ...
  /* Cycle constants where found using seperate code not shown here. The basic
   * idea was that we would run the sim until a state repeated. We'd note the
   * number of steps until repeat. Then, starting from the cycle start state,
   * we would measure the length of the cycles by stepping until we came back
   * to our cycle start state. The number of minutes to run the sim for then
   * becomes total_min = mins_until_cycle + (1e9 % cycle_size) */
  const int kMinutesUntilCycle = 446;
  const int kMinutesRemainingAfterSkippingCycles = 22;
  const int kNumMinutes =
      kMinutesUntilCycle + kMinutesRemainingAfterSkippingCycles;
  ...
  int total_resources = num_wooded_acres * num_lumberyards;
  std::cout << "Answer: " << total_resources << std::endl;

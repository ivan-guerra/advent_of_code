...
void MarkWaterLocations(VeinMap& veins, Vein& curr, const GridBounds& bounds) {
    ...
    if ((lb >= 0) && (rb >= 0)) {
        for (int i = lb + 1; i < rb; ++i) {
            ...
            if (veins.count(pos) &&
                veins[pos].type == VeinType::kRunningWater) {
...
int main(int argc, char** argv) {
    ...
    auto num_reachable_tiles =
        std::count_if(veins.cbegin(), veins.cend(), [](const auto& c) {
            return (c.second.type == VeinType::kRunningWater) ||
                   (c.second.type == VeinType::kStandingWater);
        });

    /* We subtract out grid_bounds.vert_min because we are told to ignore
     * tiles with a coordinate smaller than the smallest y coordinate in our
     * input. */
    std::cout << "Answer: " << (num_reachable_tiles - grid_bounds.vert_min)
              << std::endl;

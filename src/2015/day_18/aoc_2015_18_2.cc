...
bool IsCorner(const LightMatrix& lights, int i, int j) {
    return (((0 == i) && (0 == j)) || ((lights.size() - 1 == i) && (0 == j)) ||
            ((0 == i) && (lights[0].size() - 1 == j)) ||
            ((lights.size() - 1 == i) && (lights[0].size() - 1 == j)));
}

void UpdateLights(LightMatrix& lights) {
    for (int i = 0; i < lights.size(); ++i) {
        for (int j = 0; j < lights[0].size(); ++j) {
            if (!IsCorner(lights, i, j)) {
...
int main(int argc, char** argv) {
    ...
    /* Hardcode the corner lights to be on. */
    lights[0].front() = '#';
    lights[0].back() = '#';
    lights.back().front() = '#';
    lights.back().back() = '#';

...
int GetMinWinningLoadoutCost(const ItemList& weapons, const ItemList& armors,
    ...
    for (const Item& armor : armors) {
      for (const std::vector<Item>& rings : ring_combos) {
        ...
        if (CanWinBattle(player, enemy)) {
          min_loadout_cost = std::min(min_loadout_cost, curr_loadout_cost);
        }
      }
    }
  }
  return min_loadout_cost;
}
...
int main(int argc, char** argv) {
  ...
  std::cout << "Answer: "
            << GetMinWinningLoadoutCost(weapons, armors, rings, enemy)
            << std::endl;

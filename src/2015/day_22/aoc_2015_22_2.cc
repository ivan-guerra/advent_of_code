...
void BattleSimulation::WizardAttacksBoss(SpellType spell) {
  /* Hard mode has us losing 1HP every attack. */
  wizard_.hitpoints--;
  ...
  while (!nodes.empty()) {
    ...
    for (int i = 0; i < level; ++i) {
      ...
      if (curr_node.second.WizardWins()) {
        ...
        nodes.push(std::make_pair(SpellType::kMagicMissile, curr_node.second));
        nodes.push(std::make_pair(SpellType::kDrain, curr_node.second));
        nodes.push(std::make_pair(SpellType::kShield, curr_node.second));
        nodes.push(std::make_pair(SpellType::kPoison, curr_node.second));
        nodes.push(std::make_pair(SpellType::kRecharge, curr_node.second));
      }
    }
  }

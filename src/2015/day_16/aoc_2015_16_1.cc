...
Aunt ParseAunt(const std::string& sue) {
  ...
  ret.attrs[strip_last_char(tokens[2])] = std::stoi(strip_last_char(tokens[3]));
  ret.attrs[strip_last_char(tokens[4])] = std::stoi(strip_last_char(tokens[5]));
...
void FilterAunts(std::list<Aunt>& aunts, const AuntAttrs& target_sue_attrs) {
  ...
  for (auto it = aunts.begin(); it != aunts.end();) {
    ...
    for (const auto& [attr, value] : it->attrs) {
      if (target_sue_attrs.at(attr) != value) {
        it = aunts.erase(it);
        has_mismatch = true;
        break;
      }

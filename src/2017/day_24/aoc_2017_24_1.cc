...
int ComputeMaxStrength(ComponentSet components, Component comp) {
    ...
    for (Component new_comp : components) {
        if (IsCompatible(comp, new_comp)) {
            ...
            new_comp.strength =
                new_comp.port_a + new_comp.port_b + comp.strength;

            max_strength = std::max(
                max_strength, ComputeMaxStrength(new_components, new_comp));
        }
    }
    return max_strength;
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "Answer: " << ComputeMaxBridgeStrength(components)
              << std::endl;

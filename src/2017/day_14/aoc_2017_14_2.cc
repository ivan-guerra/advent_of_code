...
using BitMatrix = std::vector<std::bitset<kHashLen>>;
...
std::string ComputeDenseHash(const std::vector<int>& sparse_hash) {
    ...
    for (int i = 0; i < sparse_hash.size(); i += kBlockSize) {
        ...
        dense_hash << std::setw(2) << std::setfill('0') << std::hex
                   << block_hash;
    }
...
void ClearRegion(BitMatrix& matrix, int i, int j) {
...
int CountRegions(BitMatrix& matrix) {
    int num_regions = 0;
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < kHashLen; ++i) {
        ...
        matrix.push_back(KnotHashToBin(knot_hash));
    }

    std::cout << "Answer: " << CountRegions(matrix) << std::endl;

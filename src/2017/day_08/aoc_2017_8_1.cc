...
int ComputeLargestRegisterVal(const std::vector<Instruction>& instructions) {
  ...
  int max_reg_value = INT_MIN;
  for (const auto& [label, value] : registers) {
    max_reg_value = std::max(value, max_reg_value);
  }
...
int main(int argc, char** argv) {
  ...
  std::cout << "Answer: " << ComputeLargestRegisterVal(instructions)
            << std::endl;

...
class Operation {
   public:
    ...
    Operation(Operator operation, uint64_t operand_1)
    ...
    uint64_t Run(uint64_t operand) {
        operand_2_ = operand;
        return ExecOperation();
    }

    uint64_t Run(uint64_t operand_1, uint64_t operand_2) {
        operand_1_ = operand_1;
...
class MonkeyTest {
   public:
    ...
    int Run(uint64_t worry_level) const {
        return (0 == (worry_level % divisor_)) ? monkey_on_true_
                                               : monkey_on_false_;
    }

    int GetDivisor() const { return divisor_; }
...
uint64_t CalculateMonkeyBusiness(std::vector<Monkey>& monkeys) {
    ...
    uint64_t monkey_business = monkeys[num_monkeys - 1].inspection_count *
                               monkeys[num_monkeys - 2].inspection_count;

    return monkey_business;
}

uint64_t SimulateMonkeyBusiness(std::vector<Monkey>& monkeys, int num_rounds) {
    uint64_t product_of_divisors = 1ULL;
    for (const Monkey& monkey : monkeys)
        product_of_divisors *= monkey.test.GetDivisor();

    while (num_rounds--) {
        for (Monkey& monkey : monkeys) {
            int num_items = monkey.worry_levels.size();
            for (int i = 0; i < num_items; ++i) {
                uint64_t curr_worry_level = monkey.worry_levels.front();
                monkey.worry_levels.pop();

                // Modulo trick taken straight from Reddit. Uses the fact that
                // (a mod kn) mod n = a mod n
                curr_worry_level %= product_of_divisors;

                // Apply the worry level change caused by monkey inspection.
                uint64_t new_worry_level = 0;
...
std::queue<uint64_t> ParseItems(const std::string& input) {
...
Operation ParseOperation(const std::string& input) {
    ...
    std::regex word_re("\\s+");
    std::sregex_token_iterator iter(expression.begin(), expression.end(),
                                    word_re, -1);
    ...
    while (std::getline(monkey_is, line)) {
        ...
        switch (curr_line) {
            ...
            case 3:
                ...
                test = ParseMonkeyTest(test_lines[0], test_lines[1],
                                       test_lines[2]);
                break;
        }
    }
